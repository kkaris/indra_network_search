    ...
    <input
      ...
      @input="
        $emit('update:modelValue', $event.target.value),
          getExternalAutoCompleteList($event.target.value)
      "
      type="search"
      class="form-control"
...
export default {
  ...
  methods: {
    isContinuedSearch(value) {
      ...
      return (
        this.lastPrefixSearch.length > 0 &&
        ...
        Math.abs(value.length - this.lastPrefixSearch.length) <= 1
      ); // Jumps in length between searches could indicate copy pasting to replace the latest search
    },
    canSearch(value) {
      // Check conditions that allow for a search to take place
      return (
        value.length > 0 && // There is a value
        ...
        (!this.isContinuedSearch(value) || // Either started typing something new e.g. 'abc' -> 'xyz'
          (this.isContinuedSearch(value) &&
            this.autoSearchResult.length >= 100) || // or need more results
          value.endsWith(":"))
      ); // or this is an NS:ID search and only
    },
