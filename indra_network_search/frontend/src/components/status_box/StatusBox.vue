...
<template>
  <div class="container text-center">
    ...
    <p>Type: {{ errorType }}<br><i class="error-message"> {{ errorMessage }}</i></p>
  </div>
</template>
...
export default {
  ...
  computed: {
    statusCode() {
      ...
      if (this.axiosError.response && this.axiosError.response.status && this.axiosError.response.status !== 200) {
