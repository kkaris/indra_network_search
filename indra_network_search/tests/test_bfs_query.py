...
from indra_network_search.query import BreadthFirstSearchQuery, _get_edge_filter_func
...
def test_allowed_edges():
    ...
    hash_set1 = _get_edge_hash(edge=hash_bl_edge1, graph=graph, large=large, signed=signed)
    hash_set2 = _get_edge_hash(edge=hash_bl_edge2, graph=graph, large=large, signed=signed)
...
def test_bfs_search_w_blacklist():
    ...
    bfsq = BreadthFirstSearchQuery(query=hash_bl_query, hash_blacklist=set(hash_blacklist))
    bfs_gen_manual = bfs_search(g=graph, source_node=brca1.name, edge_filter=edge_filter_func)
    ...
    for p1, p2, pe in zip_longest(bfs_gen_manual, bfs_gen_query, combined_paths, fillvalue=None):
